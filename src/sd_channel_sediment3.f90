      ...
      real :: vel, veg, vel_cr, rad_curv, vel_bend, vel_rch, arc_len, prot_len
      ...
      b_exp = 12.3 / sqrt (sd_ch(ich)%ch_clay)
      b_exp = min (3.5, b_exp)
      if (vel_rch > vel_cr .and. rcurv%dep / sd_ch(ich)%chd > 0.1) then
        ebank_m = 0.00233 * (vel_rch / vel_cr) ** b_exp    !bank erosion m/yr
      else
        ebank_m = 0.
      end if
      ...
      ch_dep%sed = (1. - del_rto) * ob(iob)%hin%sed
      ch_dep%orgn = sd_ch(ich)%n_dep_enr * (1. - del_rto) * ob(iob)%hin%orgn
      ch_dep%sedp = sd_ch(ich)%p_dep_enr * (1. - del_rto) * ob(iob)%hin%sedp
      ...
      ch_trans%no3 = -conc_chng * ob(iob)%hin%no3
      ch_trans%orgn = -ch_trans%no3
      conc_chng = 1. - exp(sd_ch(ich)%p_sol_part * rcurv%ttime)
      ch_trans%solp = -conc_chng * ob(iob)%hin%solp
	  ...
	  ht2 = ht1 - fp_dep - ch_dep + bank_ero + bed_ero + ch_trans
